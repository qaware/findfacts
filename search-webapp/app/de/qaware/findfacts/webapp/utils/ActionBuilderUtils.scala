 ...
 */
class ActionBuilderUtils(queryService: QueryService, actions: ActionBuilder[Request, AnyContent])(implicit
    ec: ExecutionContext)
  extends Logging
  with Results {
   ...
   */
  def indexValid[A](index: String)(action: Action[A]): Action[A] =
    actions.async(action.parser) { request: Request[A] =>
   ...
   */
  def idValid[A](id: String)(action: Action[A]): Action[A] =
    actions.async(action.parser) { request: Request[A] =>
      if (id.isBlank) {
   ...
   */
  def filterQueryValid(action: Action[FilterQuery]): Action[FilterQuery] =
    actions.async(action.parser) {
   ...
   * @return action that can handle invalid queries
   */
  def facetQueryValid(action: Action[FacetQuery]): Action[FacetQuery] =
    actions.async(action.parser) {
