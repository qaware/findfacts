...
class TypExtractor(nameExtractor: NameExtractor) {
   ...
   */
  def prettyPrint(typ: Typ): String =
    typ match {
      // Functions
  ...
  private def prettyPrintFnArg(typ: Typ): String =
    typ match {
      case TypeTyp(PureSyntax.Fun.name, _ :: _) => s"(${prettyPrint(typ)})"
      case _ => prettyPrint(typ)
    }

  private def prettyPrintCtorArg(typ: Typ): String =
    typ match {
  ...
  private def prettyPrintSorts(sorts: List[String]): String =
    sorts match {
      case List() => ""
   ...
   */
  def referencedTypes(typ: Typ): Set[String] =
    typ match {
