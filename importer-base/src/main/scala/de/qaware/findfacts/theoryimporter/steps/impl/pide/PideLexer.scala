...
import de.qaware.findfacts.theoryimporter.dataaccess.treequery.QueryDsl.{all, ofOne, single}
import de.qaware.findfacts.theoryimporter.dataaccess.treequery.YxmlTreeQuery.{YxmlNode, body, key, keyValue, tag}
import de.qaware.findfacts.theoryimporter.dataaccess.treequery.{QueryDsl, QueryException, QueryNode}
...
import de.qaware.findfacts.theoryimporter.steps.impl.pide.PideField._
...
object PideLexer extends Parsers {
  ...
  protected def string: Parser[StringToken] =
    nodeMatches(single root ofOne thats tag(String)) ^^ (x => StringToken(extractBody(x)))
