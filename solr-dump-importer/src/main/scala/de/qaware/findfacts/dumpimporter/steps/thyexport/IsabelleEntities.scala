...
object IsabelleEntities {
  ...
  final case class TProd(name: String, args: Array[Typ]) extends Typ {
    override def toString: String = name match {
      case PureSyntax.Fun.name => s"(${args.mkString(" ⇒ ")})"
      case _ =>
        args match {
          case Array() => name
  ...
  final case class Constref(name: String, typargs: Array[Typ]) extends Term {
    override def toString(vars: IndexedSeq[String]): String = name match {
      case PureSyntax.Type.name => "Γ"
      case _ => s"<$name>"
    }
  ...
  final case class App(fun: Term, arg: Term) extends Term {
    override def toString(vars: IndexedSeq[String]): String = fun match {
      case Constref(PureSyntax.Eq.name, _) => s"${arg.toString(vars)} ≡"
      case Constref(PureSyntax.Imp.name, _) => s"${arg.toString(vars)} ⟹"
      case Constref(PureSyntax.All.name, _) => s"(⋀ ${arg.toString(vars)})"
      case Constref(PureSyntax.Conj.name, _) => s"${arg.toString(vars)} ∧"
